...
|-----|-----|-----|-----|
| imageVariant | Bun and Debian version (trixie = Debian 13, bookworm = Debian 12). Other published tags can be entered. | string | 1-trixie |
...
- Ships a `.vscode/launch.json`, copied into your project when the template is applied, with two debug configurations for that extension: `Debug Bun` runs the file open in the editor with the image's Bun binary, and `Attach to Bun` connects to an already-running process at `ws://localhost:6499/`, the default host and port of [Bun's inspector](https://bun.com/docs/runtime/debugger). Because `bun --inspect` appends a randomly generated session token to that URL, either copy the URL Bun prints into the configuration, or start the process with a fixed prefix (`bun --inspect=localhost:6499/debug app.ts`) and set `url` to match. Delete the file if your project already has its own launch configurations.
